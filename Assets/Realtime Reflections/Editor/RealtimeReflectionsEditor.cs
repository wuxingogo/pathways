...
public class RealtimeReflectionsEditor : Editor {
	...
	void OnEnable() {
		...
		for (int i=0;i<resolutions.Length;i++) resolutionsStrings[i] = resolutions[i].ToString();
	...
	public override void OnInspectorGUI() {
		...
		if (GUI.changed) EditorUtility.SetDirty (castedTarget);
		AssignMaterials ();
		myTarget.ApplyModifiedProperties ();
	}

	void AssignMaterials() { castedTarget.gameObject.GetComponent<Renderer>().materials = castedTarget.materials; }
}
