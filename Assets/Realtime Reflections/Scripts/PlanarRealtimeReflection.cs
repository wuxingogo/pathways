...
public class PlanarRealtimeReflection : MonoBehaviour {
	...
	private Hashtable m_ReflectionCameras; 						//Camera -> Camera table
	private RenderTexture m_ReflectionTexture;
	private Renderer rn;

	public PlanarRealtimeReflection() {
		m_DisablePixelLights 		= true; 					m_NormalsFromMesh 					= false;
		m_BaseClipOffsetFromMesh 	= false;					m_BaseClipOffsetFromMeshInverted 	= false;
		s_InsideRendering 			= false;					m_OldReflectionTextureSize 			= 0;
		m_TextureResolution 		= 1024;						m_OldReflectionTextureSize 			= 0;
		m_clipPlaneOffset 			= 0.07f; 					m_finalClipPlaneOffset 				= 0.0f;
		m_calculatedNormal 			= Vector3.zero;				m_ReflectLayers 					= -1;
		m_ReflectionCameras 		= new Hashtable();			m_ReflectionTexture 				= null;
	}

	public void OnAwake() { rn = GetComponent<Renderer>(); }
	...
	public void OnWillRenderObject() {
		if(!enabled || !rn || !rn.sharedMaterial || !rn.enabled) return;
		...
		if(m_BaseClipOffsetFromMesh && GetComponent<MeshFilter>() != null)
			...
			m_finalClipPlaneOffset = -(transform.position - transform.TransformPoint(GetComponent<MeshFilter>().sharedMesh.vertices[0])).magnitude + m_clipPlaneOffset;
		else m_finalClipPlaneOffset = m_clipPlaneOffset;
		...
		Material[] materials = rn.sharedMaterials;
	...
	private void UpdateCameraModes(Camera src, Camera dest) {
		...
		if (src.clearFlags == CameraClearFlags.Skybox) {
			...
			if(!sky || !sky.material) mysky.enabled = false;
			else {
				mysky.enabled = true;
	...
	private static float sgn(float a) { return (a==0.0f)?(0.0f):((a<0.0f)?(1.0f):(-1.0f)); }
