...
-- All code by Sandor Moldan, except where noted.
--
...
public class TerrainToolkit : MonoBehaviour {
	public delegate void ErosionProgressDelegate(string title,string display,int iteration,int nIterations,float complete);
	public delegate void TextureProgressDelegate(string title, string display, float complete);
	public delegate void GeneratorProgressDelegate(string title, string display, float complete);
	public GUISkin guiSkin;
	public Texture2D createIcon, erodeIcon, textureIcon, mooreIcon, vonNeumannIcon, mountainsIcon, hillsIcon, plateausIcon;
	...
	private float[,] fastErosion(float[,] heightMap, Vector2 arraySize, int iterations, ErosionProgressDelegate erosionProgressDelegate) {
		...
		for (int iter = 0; iter < iterations; iter++) {
			...
			if (erosionMode == ErosionMode.Filter) {
				...
				switch (erosionType) {
					case ErosionType.Thermal:
					title = "Applying Thermal Erosion";
					display = "Applying thermal erosion.";
					break;
					case ErosionType.Hydraulic:
					title = "Applying Hydraulic Erosion";
					display = "Applying hydraulic erosion.";
					break;
					case ErosionType.Tidal:
					title = "Applying Tidal Erosion";
					display = "Applying tidal erosion.";
					break;
					default:
					return heightMap;
				}
				float complete = (float) iter / (float) iterations;
				erosionProgressDelegate(title, display, iter, iterations, complete);
			}
		}
		return heightMap;
	}
	...
	private float[,] velocityHydraulicErosion(float[,] heightMap, Vector2 arraySize, int iterations, ErosionProgressDelegate erosionProgressDelegate) {
		...
		for (int iter = 0; iter < iterations; iter++) {
			...
			// Update progress...
			float complete = (float) iter / (float) iterations;
			erosionProgressDelegate("Applying Hydraulic Erosion", "Applying hydraulic erosion.", iter, iterations, complete);
		}
		return heightMap;
	}
	...
	private float[,] windErosion(float[,] heightMap, Vector2 arraySize, int iterations, ErosionProgressDelegate erosionProgressDelegate) {
		...
		for (int iter = 0; iter < iterations; iter++) {
			...
			// Update progress...
			float complete = (float) iter / (float) iterations;
			erosionProgressDelegate("Applying Wind Erosion", "Applying wind erosion.", iter, iterations, complete);
		}
		return heightMap;
	}
	...
	public void textureTerrain(TextureProgressDelegate textureProgressDelegate) {
		...
		try {
			...
			for (int Ty = 0; Ty < Th; Ty++) {
				...
				// float complete = (completePoints / totalPoints) * 0.6f;
				// textureProgressDelegate("Procedural Terrain Texture", "Generating height and slope maps. Please wait.", complete);
			}
			// Blend slope...
			float sBlended;
			int Px;
	...
	private float[,] generateVoronoi(float[,] heightMap, Vector2 arraySize, GeneratorProgressDelegate generatorProgressDelegate) {
		...
		for (My = 0; My < Ty; My++) {
			...
			float complete = completePoints / totalPoints;
			generatorProgressDelegate("Voronoi Generator", "Generating height map. Please wait.", complete);
		}
		// Normalise...
		for (My = 0; My < Ty; My++) {
			for (Mx = 0; Mx < Tx; Mx++) {
	...
	private float[,] generatePerlin(float[,] heightMap, Vector2 arraySize, GeneratorProgressDelegate generatorProgressDelegate) {
		...
		for (i = 0; i < perlinOctaves; i++) {
			...
			// Show progress...
			float complete = (i + 1) / perlinOctaves;
			generatorProgressDelegate("Perlin Generator", "Generating height map. Please wait.", complete);
		}
	...
	private float[,] smooth(float[,] heightMap, Vector2 arraySize, GeneratorProgressDelegate generatorProgressDelegate) {
		...
		for (int iter = 0; iter < smoothIterations; iter++) {
			...
			// Show progress...
			float complete = (iter + 1) / smoothIterations;
			generatorProgressDelegate("Smoothing Filter", "Smoothing height map. Please wait.", complete);
		}
		return heightMap;
	}
	...
	public void dummyErosionProgress(string title, string display, int iteration, int nIterations, float complete) {
		//
	}

	public void dummyTextureProgress(string title, string display, float complete) {
		//
	}

	public void dummyGeneratorProgress(string title, string display, float complete) {
		//
	}
