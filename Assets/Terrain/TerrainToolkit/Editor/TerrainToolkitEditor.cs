...
public class TerrainToolkitEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		switch (terrain.toolModeInt) {
			...
			int nTextures = 0;
			EditorGUILayout.Separator();
			if (GUI.changed) EditorUtility.SetDirty(terrain);
			GUI.changed = false;
			EditorGUILayout.BeginHorizontal();
			foreach (SplatPrototype splatPrototype in terrain.splatPrototypes) {
				...
				if (nTextures==0) splatPrototype.texture = EditorGUILayout.ObjectField(
					"Cliff texture", splatPrototype.texture, typeof(Texture2D), false) as Texture2D;
				else splatPrototype.texture = EditorGUILayout.ObjectField("Texture "+nTextures,
					splatPrototype.texture, typeof (Texture2D), false) as Texture2D;
				GUI.skin = terrain.guiSkin;
				textureRect.x += 146;
