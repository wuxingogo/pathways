/* Ben Scott * bescott@andrew.cmu.edu * 2015-08-15 * Crystal */
...
namespace PathwaysEngine.Inventory {
	public partial class Crystal : Item, IWieldable {
		...
		public float rate, rateSpread, rateCool, rateScope,
		...
		public AudioClip[] audFirings, audImpacts, audReloads;
		...
		public void Attack() {
			if (sinceShot>rate && !am.isPlaying) {
				...
				sinceShot = 0;
				if (audFirings.Length>0) localAU.PlayOneShot(audFirings[UnityEngine.Random.Range(0, audFirings.Length)]);
#endif
				switch (WeaponType) {
...
//	if (WeaponType == WeaponTypes.KinematicTrajectile) Weapon.Instantiate(Trajectile, trPos[1].position, trPos[1].rotation);
					if (WeaponType == WeaponTypes.Projectile) {
						...
						if (Physics.Raycast(transform.position,ProjectileDirection,out ProjectileHit,(int)range,weaponMask)) {
