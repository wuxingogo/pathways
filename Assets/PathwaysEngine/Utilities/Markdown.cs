...
namespace PathwaysEngine.Utilities {
    ...
    public class Markdown {
        ...
        readonly Dictionary<string, string> _htmlBlocks = new Dictionary<string, string>();
        ...
        string RunBlockGamut(string text, bool unhash = true, bool createParagraphs = false) {
            text = DoHeaders(text);
        ...
        string RunSpanGamut(string text) {
        ...
        string FormParagraphs(string text,bool unhash = true,bool createParagraphs = false) {
        ...
        string StripLinkDefinitions(string text) {
            return _linkDef.Replace(text, new MatchEvaluator(LinkEvaluator));
        }

        string LinkEvaluator(Match match) {
        ...
        string HashHTMLBlocks(string text) { return _blocksHtml.Replace(text, new MatchEvaluator(HtmlEvaluator)); }

        string HtmlEvaluator(Match match) {
        ...
        List<Token> TokenizeHTML(string text) {
            int pos = 0;
            int tagStart = 0;
        ...
        string SaveFromAutoLinking(string s) { return s.Replace("://", AutoLinkPreventionMarker); }

        string AnchorRefEvaluator(Match match) {
        ...
        string AnchorRefShortcutEvaluator(Match match) {
            string wholeMatch = match.Groups[1].Value;
            string linkText = SaveFromAutoLinking(match.Groups[2].Value);
        ...
        string AnchorInlineEvaluator(Match match) {
            string linkText = SaveFromAutoLinking(match.Groups[2].Value);
        ...
        string EscapeImageAltText(string s) {
            s = EscapeBoldItalic(s);
            s = Regex.Replace(s, @"[\[\]()]", m => _escapeTable[m.ToString()]);
            return s;
        }

        string ImageReferenceEvaluator(Match match) {
            string wholeMatch = match.Groups[1].Value;
        ...
        string ImageInlineEvaluator(Match match) {
        ...
        string ImageTag(string url, string altText, string title)
        {
        ...
        string SetextHeaderEvaluator(Match match)
        {
        ...
        string AtxHeaderEvaluator(Match match)
        {
        ...
        /// - - -
        /// </remarks>
        string DoHorizontalRules(string text)
        {
        ...
        string ListEvaluator(Match match)
        {
        ...
        /// </summary>
        string ProcessListItems(string list, string marker)
        {
        ...
        /// </summary>
        string DoCodeBlocks(string text)
        {
            text = _codeBlock.Replace(text, new MatchEvaluator(CodeBlockEvaluator));
            return text;
        }

        string CodeBlockEvaluator(Match match)
        {
        ...
        /// </summary>
        string DoCodeSpans(string text)
        {
        ...
        string CodeSpanEvaluator(Match match)
        {
        ...
        string DoItalicsAndBold(string text) {
#if HTML
            ...
            if (_strictBoldItalic) {
                if (_asteriskIntraWordEmphasis) {
                    text = _semiStrictBold.Replace(text, "$1<b>$3</b>");
                    text = _semiStrictItalic.Replace(text, "$1<i>$3</i>");
                } else {
                    text = _strictBold.Replace(text, "$1<b>$3</b>");
                    text = _strictItalic.Replace(text, "$1<i>$3</i>");
                }
            } else {
                text = _bold.Replace(text, "<b>$2</b>");
                text = _italic.Replace(text, "<i>$2</i>");
            } return text;
#endif
        }
        ...
        /// </summary>
        string DoHardBreaks(string text) {
            if (_autoNewlines)
        ...
        /// </summary>
        string DoBlockQuotes(string text) {
            return _blockquote.Replace(text, new MatchEvaluator(BlockQuoteEvaluator));
        }

        string BlockQuoteEvaluator(Match match) {
        ...
        string BlockQuoteEvaluator2(Match match) {
        ...
        string DoAutoLinks(string text) {
            if (_autoHyperlink) {
        ...
        string Outdent(string block) { return _outDent.Replace(block, ""); }


        string EncodeEmailAddress(string addr) {
        ...
        string EncodeCode(string code) { return _codeEncoder.Replace(code,EncodeCodeEvaluator);}

        string EncodeCodeEvaluator(Match match) {
            switch (match.Value) {
        ...
        string EncodeAmpsAndAngles(string s) {
            s = _amps.Replace(s, "&amp;");
            s = _angles.Replace(s, "&lt;");
            return s;
        }
        ...
        string EscapeBackslashes(string s) { return _backslashEscapes.Replace(s, new MatchEvaluator(EscapeBackslashesEvaluator)); }

        string EscapeBackslashesEvaluator(Match match) { return _backslashEscapeTable[match.Value]; }

        static Regex _unescapes = new Regex("\x1A" + "E\\d+E");//, RegexOptions.Compiled);

        string Unescape(string s) { return _unescapes.Replace(s, new MatchEvaluator(UnescapeEvaluator)); }

        string UnescapeEvaluator(Match match) { return _invertedEscapeTable[match.Value]; }

        string EscapeBoldItalic(string s) {
        ...
        /// </summary>
        string EscapeSpecialCharsWithinTagAttributes(string text) {
