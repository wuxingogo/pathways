...
namespace PathwaysEngine.Movement {
	public class PrototypeGimbal : MonoBehaviour {
		...
		double sigma, cutoff;
		double[] rotArray;
	 	public PrototypeMotor childMotor;
		...
		public void Start() {
			...
			//childMotor = childPlayer.GetComponent<PrototypeMotor>();
		...
		public void FixedUpdate() {
			...
			} if (!childMotor.isGrounded) transform.RotateAround(
		...
		public void PlayerReplace() {
			...
			temp.GetComponent<Rigidbody>().velocity = gameObject.GetComponentInChildren<PrototypeMotor>().velocity;
			temp.parent = null;
			isDead = true;
			Destroy(gameObject);
#endif
			...
			deadtemp.GetComponent<Rigidbody>().velocity = childPlayer.GetComponent<PrototypeMotor>().velocity;
