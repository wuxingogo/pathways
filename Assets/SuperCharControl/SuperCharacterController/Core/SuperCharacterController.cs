...
public class SuperCharacterController : MonoBehaviour {
    ...
    public struct Ground {
        ...
        public Ground(RaycastHit hit,RaycastHit nearHit,RaycastHit farHit,
        RaycastHit secondaryHit,SuperCollisionType superCollisionType,
    ...
    [SerializeField]
    CollisionSphere[] spheres = new CollisionSphere[3] {
    ...
    void SingleUpdate() {
        ...
        Transform clampedTo = currentlyClampedTo != null
            ? currentlyClampedTo : currentGround.transform;
        ...
        clampedTo = currentlyClampedTo != null
            ? currentlyClampedTo : currentGround.transform;
    ...
    public bool IsClamping() { return clamping; }
    ...
    private bool SimulateSphereCast(Vector3 groundNormal, out RaycastHit hit) {
        ...
        if (!Mathf.Approximately(groundAngle, 0)) {
            ...
            hit.distance -= Tolerance;// Remove the tolerance from the distance travelled
            return true;
        } else return false;
    }
            ...
            foreach (Collider col in Physics.OverlapSphere((SpherePosition(sphere)), radius, Walkable)) {
                ...
                if (contactPoint != Vector3.zero) {
                    if (debugPushbackMesssages)
                        DebugDraw.DrawMarker(contactPoint,2f,Color.cyan,0f,false);
                    Vector3 v = contactPoint - position;
                    if (v != Vector3.zero) {
                        // Cache the collider's layer so that we can cast against it
                        int layer = col.gameObject.layer; // Debug.Log(TemporaryLayerIndex);
                        ...
                        contact = true;
                        transform.position += v; // Debug.Log(TemporaryLayer);
                        col.gameObject.layer = TemporaryLayerIndex;
    ...
    public void ClearIgnoredColliders() { ignoredColliders.Clear(); }
    ...
    public class SuperGround {
        ...
        GroundHit primaryGround, nearGround, farGround, stepGround, flushGround;
        ...
        public void ProbeGround(Vector3 origin, int iter) {
            ...
            if (Physics.SphereCast(o, smallerRadius, down, out hit, Mathf.Infinity, walkable)) {
                ...
                if (Vector3.Distance(Math3d.ProjectPointOnPlane(controller.up, controller.transform.position, hit.point), controller.transform.position) < TinyTolerance) return;
                ...
                if (Vector3.Angle(nearHit.normal, up) > superColType.StandAngle || nearHit.distance > Tolerance) {
                    //print (nearHit);
                    //print (nearHit.collider.gameObject);
            ...
            } else Debug.LogWarning("[SuperCharacterComponent]: No ground was found below the player; player has escaped level");
        }
        ...
        public bool IsGrounded(bool currentlyGrounded, float distance, out Vector3 groundNormal) {
            ...
            if (farGround != null && !OnSteadyGround(farGround.normal, primaryGround.point)) {
                ...
                if (nearGround != null && nearGround.distance < distance && Vector3.Angle(nearGround.normal, controller.up) < superCollisionType.StandAngle && !OnSteadyGround(nearGround.normal, nearGround.point)) {
                    ...
                    return true;
                } if (stepGround!=null && stepGround.distance<distance // Check if we are on a step or stair
                && Vector3.Angle(stepGround.normal,controller.up)<superCollisionType.StandAngle) {
                    groundNormal = stepGround.normal;
                    return true;
                } return false;
            } groundNormal = (farGround!=null)?(farGround.normal):(primaryGround.normal);
            return true;
        }
        ...
        public Vector3 PrimaryNormal() { return primaryGround.normal; }
        ...
        public float HitDistance() { return primaryGround.distance; }

        public float Distance() { return primaryGround.distance; }
