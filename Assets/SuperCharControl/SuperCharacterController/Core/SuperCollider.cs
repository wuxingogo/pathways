...
public static class SuperCollider {
    ...
    public static Vector3 ClosestPointOnSurface(Collider collider, Vector3 to, float radius) {
        if (collider is BoxCollider)
            ...
            if (bsp != null) return bsp.ClosestPointOn(to, radius);
            BruteForceMesh bfm = collider.GetComponent<BruteForceMesh>();
            if (bfm != null) return bfm.ClosestPointOn(to);
        }
        return Vector3.zero;
    }
    ...
    public static Vector3 ClosestPointOnSurface(BoxCollider collider, Vector3 to) {
        var ct = collider.transform; // Cache the collider transform
        ...
        localNorm += collider.center; // Now we undo our transformations
        return ct.TransformPoint(localNorm); // Return resulting point
    }
