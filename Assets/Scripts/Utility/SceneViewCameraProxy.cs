...
public class SceneViewCameraProxy : MonoBehaviour {
    ...
    private Component[] GetComponents() {
        ...
        if (result != null && result.Length > 1) {
            ...
            if (hasCamera) result = UseReferenceCamera ? result.Except(new[] {ReferenceCamera}).ToArray() : result.Except(new[] {GetComponent<Camera>()}).ToArray();
        } return result;
    }
    ...
    private void UpdateComponents() {
        ...
        if (components!=null && components.Length>1) {
            ...
            for (var i=0;i<components.Length;i++) {
                ...
                Debug.Log(cameraGo.GetComponent(cType)+"#yolo");
