...
public class DictionaryF <K, L, V> {
	...
	public V this [L subKey] {
		get {
			V item;
			if (TryGetValue(subKey, out item)) return item;
	...
	public V this [K primaryKey] {
		get {
			V item;
			if (TryGetValue(primaryKey, out item)) return item;
	...
	public void Associate ( L subKey, K primaryKey ) {
		...
		try{
			if (!baseDictionary.ContainsKey(primaryKey)) throw new KeyNotFoundException(string.Format("The base dictionary does not contain the key '{0}'", primaryKey));
	...
	public bool TryGetValue ( L subKey, out V val ) {
		...
		try { if (subDictionary.TryGetValue(subKey, out primaryKey)) return baseDictionary.TryGetValue(primaryKey, out val); }
		finally { readerWriterLock.ExitWriteLock(); }
	 	return false;
	}
	...
	public bool ContainsKey ( L subKey ) { V val;	 return TryGetValue(subKey, out val); }
 
	public bool ContainsKey ( K primaryKey ) { V val; return TryGetValue(primaryKey, out val); }
