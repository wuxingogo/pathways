...
public class UnityLock : EditorWindow
{
  ...
  //    if (EditorPrefs.GetBool(addUndoRedoPrefKey)) Undo.RegisterSceneUndo("Lock Object");
   ...
   //     if (EditorPrefs.GetBool(addUndoRedoPrefKey)) Undo.RegisterSceneUndo("Unlock Object");
        go.hideFlags &= ~HideFlags.NotEditable;
        foreach (Component comp in go.GetComponents(typeof(Component)))
        {
            if (!(comp is Transform))
